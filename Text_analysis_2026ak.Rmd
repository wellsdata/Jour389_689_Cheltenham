...
ggsave("distribution_by_year.png", plot = p, width = 10, height = 6, dpi = 300)
```
```{r}
...
Article_506
```{r}
wrapped_text <- strwrap(as.character(afro_sun$full_text[315]), width = 60)
...
```{r}
authors <- afro_sun |>
  select(authors, pubtitle) |> 
  group_by(authors, pubtitle) |>
  count() |> 
  arrange(desc(pubtitle))
```

```{r}
committed <- sentencing_docs |> 
  select(keyword, pattern, pubtitle) |> 
  group_by(keyword, pubtitle) |> 
  count() |> 
  arrange(desc(pubtitle))
```
...
ggplot(top_20_bigrams, aes(x = reorder(bigram, n), y = n, fill=n)) +
  ...
  coord_flip() +  
  labs(title = "Top Two-Word phrases in afro_sun coverage",
...
sentiment <- afro_sun |> 
  ...
  unnest_tokens(tokens, clean_text) |> 
